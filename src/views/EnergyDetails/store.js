...
class EnergyDetailsStore {
  @observable isLoading = false

  @observable search = { type: '', position: '' }
  ...
  @computed get listCtd() {
    const { energyType } = dicStore

    return this.list.map(({ type, ...rest }) => {
      const findObj = energyType.find(({ value }) => +value === +type)
...
export default new EnergyDetailsStore()
